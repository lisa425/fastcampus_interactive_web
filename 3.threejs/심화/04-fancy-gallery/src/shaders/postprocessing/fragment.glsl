...
//   newUV.x -= (newUV.x - 0.5) * side * 0.05; //-0.5는 평균 맞추는 용
...
//   vec4 tex = texture2D(tDiffuse, newUV);
//   tex += noise;
//   vec4 sideColor = vec4(0.0, 0.0, 1.0, 1.0);
//   gl_FragColor = tex;
// }
...
void main() {
  ...
  tex += blending * uScrolling; //blending의 정도를 정해주기 위해 uScrolling 사용
  vec4 sideColor = vec4(0.0, 0.0, 1.0, 1.0);
  gl_FragColor = tex;
}